...
public final class OutlineConfigManager {
    ...
    public static int getCurrentFillArgbColor() {
        OutlineConfig current = config;
        int alpha = clamp(current.fillAlpha, 0, 255);
        if (!current.rainbowEnabled) {
            return current.fillToArgb();
        }
        float cycles = Math.max(0.01F, current.rainbowCyclesPerSecond);
        float seconds = System.nanoTime() / 1_000_000_000.0F;
        float hue = (seconds * cycles) % 1.0F;
    ...
    private static OutlineConfig sanitize(OutlineConfig outlineConfig) {
        ...
        outlineConfig.fillAlpha = clamp(outlineConfig.fillAlpha, 0, 255);
        outlineConfig.fillRed = clamp(outlineConfig.fillRed, 0, 255);
        outlineConfig.fillGreen = clamp(outlineConfig.fillGreen, 0, 255);
        outlineConfig.fillBlue = clamp(outlineConfig.fillBlue, 0, 255);
